...
class mono_dataset(data.Dataset):
    ...
    def __init__(self,
        ...
        for i in range(self.num_scales):
            ...
            self.resize_mask = transforms.Resize((self.height, self.width), interpolation=Image.NEAREST)
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            if i == 's':
                ...
                inputs[('instance_mask', i, -1)] = self.get_instance_mask(other_side, set_type, city, seq_index, frame_index, do_flip)

                inputs[('instance_mask_x', i, -1)] = self.encode_instancemap(inputs[('instance_mask', i, -1)],
                                                                            width_or_height='width')
                inputs[('instance_mask_y', i, -1)] = self.encode_instancemap(inputs[('instance_mask', i, -1)],
                ...
                inputs[('instance_mask', i, -1)] = self.get_instance_mask(side, set_type, city, seq_index, f_i_string, do_flip)
        ...
        for i in self.frame_idxs:
            ...
            del inputs[('instance_mask', i, -1)]
            del inputs[('instance_mask_x', i, -1)]
            del inputs[('instance_mask_y', i, -1)]
    ...
    def get_instancemap(self, side, set_type, city, seq_index, frame_index, do_flip, x_or_y='x'):
        raise NotImplementedError